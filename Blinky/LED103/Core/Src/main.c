...
#include <stdint.h>
#include "main.h"
extern uint32_t getSysTickCounter();
void SystemClock_Config();
int delay_mS (uint32_t millis)
{
	uint32_t strt= getSysTickCounter();
	while((getSysTickCounter()-strt)<millis);
	return 0;
}
int main(void)
{
	SystemClock_Config();
	SysTick_Config((uint32_t)180*1000);

	RCC->AHB1ENR |= RCC_AHB1ENR_GPIOGEN; //enable Port G
	GPIOG->MODER |= GPIO_MODER_MODE13_0; // output mode (01)
	GPIOG->OTYPER &= ~(GPIO_OTYPER_OT13); // Push-Pull mode
